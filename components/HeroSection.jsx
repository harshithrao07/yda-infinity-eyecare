...
export default function HeroContent() {
  ...
  return (
    <main className="relative h-screen grid grid-cols-2 items-center gap-8 bg-[#F5F5F7] overflow-hidden mt-5">
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="h-full w-full bg-[repeating-linear-gradient(to_right,#9ca3af_0px,#9ca3af_2px,transparent_2px,transparent_100px),repeating-linear-gradient(to_bottom,#9ca3af_0px,#9ca3af_2px,transparent_2px,transparent_100px)]" />
      </div>
      ...
      {/* Left Column */}
      <div className="flex flex-col justify-center text-left pl-12">
        {/* Highlight pill */}
        <motion.div
          ...
          <span className="italic instrument inline-block font-medium">
            {["Vision"].map((w) => (
              <motion.span
                key={w}
                variants={word}
                className={`inline-block mr-4 ${instrument.className}`}
              >
                {w}
              </motion.span>
            ))}
        ...
        >
          At{" "}
          <span
            className={`font-bold italic ${instrument.className} text-black`}
          >
            Infinity Eye Care
          </span>
          , we combine advanced technology with compassionate care to help you
          see the world clearly and comfortably. Your eyes deserve the best, and
          we’re here to provide it—today and always.
        </motion.p>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          >
            <Button
              variant="outline"
              size="lg"
              className="rounded-full px-7 cursor-pointer border-gray-400 border-1 group hover:border-gray-600 hover:shadow-lg transition-all duration-300 hover:bg-gray-50"
              onMouseEnter={() => {
                controls.start({
      ...
      <motion.div
        className="h-3/4 border-black border-y-6 border-l-6"
        initial={{ filter: "blur(10px)", opacity: 0, y: 8 }}
        whileInView={{ filter: "blur(0px)", opacity: 1, y: 0 }}
        viewport={{ once: true, amount: 0.6 }}
        transition={{
